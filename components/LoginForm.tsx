...
function getAccountName(email: string) {
  return email.toLowerCase().includes("elite") ? "Elite" : "OroActive";
}
...
export function LoginForm() {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    window.localStorage.setItem("oroactive-account-name", getAccountName(email));
  ...
  return (
    <form onSubmit={handleSubmit} className="glass w-full max-w-md rounded-[2rem] p-8">
      ...
      <label className="mt-6 grid gap-2 text-sm font-bold text-warm/70">
        Email
        <input
          className="rounded-2xl border border-white/10 bg-black/40 px-4 py-4 outline-none focus:border-orange"
          type="email"
          value={email}
